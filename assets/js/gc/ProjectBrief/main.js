...
$(document).ready(function() {
  ...
  $('#upload-btn').click(function() {
    $.post(Routing.generate('asset_upload', {'id': this.project_id}) + '?' + $('#session_name').val() + '=' + $('#session_id').val(), {
  ...
  $('.thumbnail button.close').on('click', function() {
    ...
    console.log(Routing.generate('asset_delete', {'id': this.project_id, 'aid': asset_id}));
    $.post(Routing.generate('asset_delete', {'id': this.project_id, 'aid': asset_id}), function(data) {
      if (data.OK === '1') {
